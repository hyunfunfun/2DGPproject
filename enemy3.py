...
class Idle:
    ...
    @staticmethod
    def draw(enemy):
        enemy.idle_image.clip_composite_draw(int(enemy.frame) * 120, 0, 70, 110,0,'h', enemy.x, enemy.y,  enemy.size,  enemy.size)
...
class Attack_ready:
    ...
    @staticmethod
    def draw(enemy):
        enemy.attack_ready_image.clip_composite_draw(int(enemy.frame) * 120, 0, 80, 90,0,'h', enemy.x, enemy.y,  enemy.size,  enemy.size)
...
class Enemy3:
    ...
    def __init__(self):
        ...
        self.idle_image = load_image('./resource\\character\\enemy3\\enemy3_idle.png')
        self.attack_ready_image = load_image(
            './resource\\character\\enemy3\\enemy3_attack_ready.png')
        self.retreat_image = load_image(
            './resource\\character\\enemy3\\enemy3_retreat.png')
        self.defense_image = load_image('./resource\\character\\enemy3\\enemy3_defense.png')
        self.attack_image = load_image('./resource\\character\\enemy3\\enemy3_attack.png')
        self.die_image = load_image('./resource\\character\\enemy3\\enemy3_die.png')
